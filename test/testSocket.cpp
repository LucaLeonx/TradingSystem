...
int main(int, char **) {
  ...
  auto tcpServerRecvCallback = [&](TCPSocket *socket, Nanos rx_time) noexcept {
    ...
    const std::string reply = "TCPServer received msg:" + std::string(socket->recv_buffer_.data(), socket->recv_next_valid_idx_);
    socket->recv_next_valid_idx_ = 0;
  ...
  auto tcpClientRecvCallback = [&](TCPSocket *socket, Nanos rx_time) noexcept {
    const std::string recv_msg = std::string(socket->recv_buffer_.data(), socket->recv_next_valid_idx_);
    socket->recv_next_valid_idx_ = 0;

    logger_.log("TCPSocket::defaultRecvCallback() socket:% len:% rx:% msg:%\n",
                socket->fd_, socket->recv_next_valid_idx_, rx_time, recv_msg);
  };
  ...
  for (size_t i = 0; i < clients.size(); ++i) {
    clients[i] = std::make_unique<TCPSocket>(logger_);
