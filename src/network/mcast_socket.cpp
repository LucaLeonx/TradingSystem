...
namespace trading {
    ...
    auto McastSocket::init(const std::string &ip, const std::string &iface, int port, bool is_listening) -> int {
        const SocketConfigs socket_cfg{ip, iface, port, true, is_listening, false};
    ...
    bool McastSocket::join(const std::string &ip) {
        return trading::join(socket_fd_, ip);
    }
    ...
    auto McastSocket::sendAndRecv() noexcept -> bool {
        ...
        logger_.log("%:% %() % read socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_), socket_fd_,
        ...
        logger_.log("%:% %() % send socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_), socket_fd_, n);
        }
